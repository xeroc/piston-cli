...
;Variables
  ...
  !define PRODUCT_NAME "Piston"
  !define PRODUCT_WEB_SITE "https://github.com/xeroc/piston"
  !define PRODUCT_PUBLISHER "ChainSquad GmbH"
...
;  !define MUI_ICON "..\..\icons\piston.ico"
...
;  File "..\..\icons\piston.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\piston.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\piston.exe" "" "$INSTDIR\piston.exe" 0

  ;Links bitcoin: URI's to piston
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\piston.ico, 0$\""
  ;WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\piston.exe$\" $\"%1$\""
  ...
  ;WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\piston.ico"
